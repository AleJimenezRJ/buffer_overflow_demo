...
int VSocket::MakeConnection( const char * hostip, int port ) {
   ...
   if (IPv6) {
      ...
      if ( -1 == st ) {
         throw std::runtime_error("Error while attempting to create conection");
      }
   } else {    // for IPV4
...
/**
  ...
  *
 **/
int VSocket::MakeConnection( const char * hostip, const char * service ) {
...
int VSocket::Bind( int port ) {
   ...
   if (IPv6){
      ...
      st = bind(idSocket, (struct sockaddr*)&addr6, sizeof(addr6));
      if (st == -1) {
         perror("Bind failed");
